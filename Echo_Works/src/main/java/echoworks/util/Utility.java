...
public class Utility {
	...
	public static String stripTag(String source) {
		...
		/*
		 * //Matcher.find() : Matcher 객체에 저장된 정보(정규표현식과 문자열)를 사용해 문자열에서 //정규표현식과 같은 패턴의
		 * 문자열을 찾아 논리값을 반환하는 메소드 // => 정규표현식과 같은 패턴의 문자열이 여러개 있는 경우 반복문을 사용해 차례대로 검색 처리
		 * while(matcher.find()) { //Matcher.group() : Matcher 객체에 저장된 정보(정규표현식과 문자열)를
		 * 사용해 문자열에서 //정규표현식과 같은 패턴의 문자열을 반환하는 메소드 String group=matcher.group();
